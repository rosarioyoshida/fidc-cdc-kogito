...
type CessaoDetailErrorProps = {
  error: Error & { digest?: string };
  reset: () => void;
};

export default function CessaoDetailError({
  error,
  reset
}: CessaoDetailErrorProps) {
  return (
    <main className="mx-auto flex min-h-[60vh] max-w-3xl flex-col items-start justify-center gap-4 px-6">
      ...
      <h1 className="text-3xl font-semibold text-text">Falha ao carregar a cessao</h1>
      <p className="max-w-2xl text-sm leading-6 text-text-subtle">
        {error.message || "Nao foi possivel consultar o backend neste momento."}
      </p>
